...
script "installing ruby-build to ~/Developer" do
  ...
  code <<-EOS
    ...
    if [[ ! -x ~/Developer/bin/ruby-build ]]; then
      git clone git://github.com/sstephenson/ruby-build.git #{Dir.tmpdir}/ruby-build >> ~/.snuggie/ruby.log
      cd #{Dir.tmpdir}/ruby-build && /usr/bin/env PREFIX=~/Developer ./install.sh >> ~/.snuggie/ruby.log
    fi
  EOS
end
...
script "installing basic gems" do
  ...
  code <<-EOS
    source ~/.snuggie.profile
    gem install bundler heroku snuggie
